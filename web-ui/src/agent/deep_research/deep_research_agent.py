...
from src.tools.stepsLog_tool import SaveStepLogInput, save_step_log
...
def create_save_to_excel_tool() -> StructuredTool:
    return StructuredTool.from_function(
        coroutine=save_to_excel,
        name="save_to_excel",
        description="Save data to an Excel file at the specified path, handling various data formats (e.g., dictionaries, lists, strings).",
        args_schema=SaveExcelInput,
    )

def create_save_step_log_tool() -> StructuredTool:
    return StructuredTool.from_function(
        coroutine=save_step_log,
        name="save_step_log",
        description="""Save step execution details to Excel for logging and debugging purposes. 
        Captures output data, errors, status, and metadata for each processing step.
        Save the step number its description and its status like got error or completed etc.
        Automatically organizes logs in a dedicated 'step_logs' directory with timestamped filenames.""",
        args_schema=SaveStepLogInput,
    )
...
async def research_execution_node(state: DeepResearchState) -> Dict[str, Any]:
    ...
    Use the tool save_step_log to save each steps as log in excel.
    """
    ...
    # Get the save_step_log tool from available tools
    save_log_tool = next((t for t in tools if t.name == "save_step_log"), None)
    if not save_log_tool:
        logger.error("save_step_log tool not found in available tools")
        return {"error_message": "Step logging tool not available"}
    ...
    try:
        ...
        # After executing the step, automatically save the log
        log_input = SaveStepLogInput(
            step_name=f"Step_{current_step['step']}_{current_step['task'][:50]}",
            output_data=current_step.get("result_summary"),
            error_data=state.get("error_message"),
            status=current_step["status"],
            additional_info={
                "task_id": task_id,
                "step_number": current_step["step"],
                "original_task": current_step["task"],
                "executed_tools": executed_tool_names if 'executed_tool_names' in locals() else []
            }
        )
        
        # Save the step log
        log_result = await save_log_tool.ainvoke(log_input.dict())
        if log_result.get("status") != "success":
            logger.error(f"Failed to save step log: {log_result.get('error')}")
        ...
        try:
            error_log_input = SaveStepLogInput(
                step_name=f"Step_{current_step['step']}_ERROR",
                error_data=str(e),
                status="failed",
                additional_info={
                    "task_id": task_id,
            ...
            await save_log_tool.ainvoke(error_log_input.dict())
...
class DeepResearchAgent:
    ...
    async def _setup_tools(
        ...
        tools = [
            ...
            ReadFileTool(),
            ListDirectoryTool(),
            create_save_to_excel_tool(),
            create_save_step_log_tool(),
